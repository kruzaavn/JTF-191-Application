<template>
  <v-container>
    ...
    >
      ...
      <v-list>
        ...
        >
          ...
          >
            <v-list-item-content>
              <v-list-item-title v-if="getQualModule(moduleID)"
    ...
    <v-card v-if="documentation">
      <v-card-title>{{documentation.name}}
        <v-spacer></v-spacer>
...
export default {
  ...
  methods: {
    ...
    setIframeHeight() {
      ...
      } else if (['slides', 'spreadsheet', 'video'].includes(this.documentation.documentation_type)) {
        this.iframeHeight = window.innerHeight * .8
      }
    },
    setIframeLink() {
      if (this.documentation.documentation_type === 'document') {
    ...
    getQualModule(moduleID) {
      return this.qualificationModules.find(qualModule => qualModule.id === moduleID)
    },
    setDocumentation(moduleID) {
      this.documentation =  this.getQualModule(moduleID)
      router.push({name: 'Qualification', params: {qualificationModule: this.documentation.name}})
    },
    clearDocumentation() {
      this.documentation = null
      this.qualificationModule = 'list'
      router.push({name: 'Qualification', params: {qualificationModule: this.qualificationModule}})
    }
  },
  mounted() {
    this.getQualifications()
    if (this.qualificationModule !== 'list' && this.qualificationModules) {
      this.documentation = this.qualificationModules.find(qualModule => qualModule.name === this.qualificationModule)
    }
  }
}
</script>
